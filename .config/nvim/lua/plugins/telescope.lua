return {
  ...
  config = function()
    ...
    vim.keymap.set("n", "<leader>fg", "<cmd>Telescope live_grep<cr>", { desc = "Live grep in directory" })

    vim.keymap.set("n", "<leader>/", "<cmd>Telescope current_buffer_fuzzy_find<cr>", { desc = "Fuzzy find in buffer" })

    vim.keymap.set("n", "<leader>fc", "<cmd>Telescope treesitter<CR>", { desc = "Find code object" })
